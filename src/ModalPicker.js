...
import { Dimensions, KeyboardAvoidingView, Modal, Platform, StatusBar, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
...
/* #region Utils */
const get: (object: any, pathToNestedProperty: (string | number)[], defaultValue?: any) => any = (object, pathToNestedProperty, defaultValue = undefined) =>
  pathToNestedProperty.reduce((xs, x) => (xs && xs[x] ? xs[x] : defaultValue), object);
    ...
    if (Platform.OS === 'ios') {
      ...
      let pickerView = (
        <View style={{ minHeight: 44, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderWidth: 1, borderColor: 'lightgray', ...containerStyle }}>
          ...
          <Icon name='arrow-drop-down' color='gray' />
        </View>
      );
      ...
      return (
        <View>
          ...
          <Modal visible={iosModalVisible} animationType='fade' onRequestClose={() => this.setState({ iosModalVisible: false })} transparent>
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : null} style={styles.modalContainerBackgroundView}>
